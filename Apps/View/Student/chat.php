...
if (isset($_GET['search'])) {
    ...
    $stmt = $db_conn->prepare("CALL SearchMentorByName(?)");
...
} elseif (isset($_GET['search-by-status'])) {
    $status = htmlspecialchars($_POST['status']);

    $stmt = $db_conn->prepare("CALL SearchMentorByStatus(?)");
...
} elseif (isset($_GET['search-by-subject'])) {
    $subject = htmlspecialchars($_POST['subject']);

    $stmt = $db_conn->prepare("CALL SearchMentorBySubject(?)");
    ...
    $sql = mysqli_query($db_conn, "CALL ShowAllMentors()");
    $mentors = [];
    ...
    if ($sql) {
        ...
        echo "Error executing the query: " . mysqli_error($db_conn);
    }
}
        ...
        <main>
            <div class="filter-banner">
                ...
                <div class="filters">
                    ...
                    <form id="statusForm" action="chat.php?search-by-status" method="post">
                        <div class="radio-button">
                            <input type="radio" id="teacherStatus" name="status" value="GURU">
                            <label for="teacherStatus">Guru</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="collegeStatus" name="status" value="MAHASISWA">
                ...
                <div class="filters">
                    ...
                    <form id="subjectForm" action="chat.php?search-by-subject" method="post">
                        <div class="radio-button">
                            <input type="radio" id="s-bahasa" name="subject" value="BINDO">
                            <label for="s-bahasa">B. Indonesia</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-english" name="subject" value="BINGGRIS">
                            <label for="s-english">B. Inggris</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-math" name="subject" value="MATEMATIKA">
                            <label for="s-math">Matematika</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-physics" name="subject" value="FISIKA">
                            <label for="s-physics">Fisika</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-chem" name="subject" value="KIMIA">
                            <label for="s-chem">Kimia</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-biology" name="subject" value="BIOLOGI">
                            <label for="s-biology">Biologi</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-eco" name="subject" value="EKONOMI">
                            <label for="s-eco">Ekonomi</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-sosio" name="subject" value="SOSIOLOGI">
                            <label for="s-sosio">Sosiologi</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-history" name="subject" value="SEJARAH">
                            <label for="s-history">Sejarah</label><br>
                        </div>
                        <div class="radio-button">
                            <input type="radio" id="s-geo" name="subject" value="GEOGRAFIS">
            ...
            <div class="search-page">
                <div class="search-bar">
                    <form action="chat.php?search" method="post">
                        <input id="searchMentorByName" name="searchMentorByName" type="search" placeholder="Cari Nama Mentor" aria-label="search" method="post" autocomplete='off'>
                        <button type="submit">
                            ...
                            <?php foreach ($mentors as $mentor): ?>
                                <div class="mentor-card">
                                    <div class="mentor-photo">
                                        <img src="data:image/png;base64," alt="Mentor Photo">
                                    </div>
                                    <div class="mentor-desc">
                                        <div class="top-desc">
                                            <p id="mentorName"><?php echo $mentor['MENTOR_NAME']; ?></p>
                                            <p id="mentorSubject"><?php echo $mentor['MENTOR_SUBJECT']; ?></p>
                                            <p id="mentorStatus"><?php echo $mentor['MENTOR_STATUS']; ?></p>
                                        </div>
                                        <div class="bottom-desc">
                                            <div class="bottom-desc-left">
                                                <p id="mentorRating"><?php echo $mentor['MENTOR_RATING']; ?></p>
                                                <p id="mentorPrice">Rp. <?php echo $mentor['MENTOR_PRICE']; ?></p>
                                            </div>
                                            <div class="right-button">
                                            <form action="../../Routes.php?getmentor" method="post">
                                                <input type="hidden" name="mentorID" value="<?= $mentor['USERS_ID']; ?>">
