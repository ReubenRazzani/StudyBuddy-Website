...
try {
    $stmt = $db_conn->prepare("CALL GetUserSchedule(?)");
    $stmt->bind_param("i", $_SESSION['idStudent']);
    $stmt->execute();
    ...
    <div class="inbox-content">
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <button id="rate" onclick="showRateForm()">Beri Ulasan</button>
                    </td>
                </tr>
                <?php
                    $rowCount++;
        ...
        <div id="topupModal" class="modal">
            <div class="modal-content">
                <span class="close" onclick="closeRateForm()">&times;</span>
                <h5>Persetujuan</h5>
                <form id="topupForm" action="../../Routes.php?rate" method="post">
                    <input type="hidden" name="scheduleID" id="scheduleID" value="<?= $row['SCHEDULE_ID']; ?>">
                    <input type="number" name="rate" placeholder="Rating 1 - 5">
                    <button type="submit" value="rate">Kirim</button>
                </form>
            </div>
        </div>
    </div>
